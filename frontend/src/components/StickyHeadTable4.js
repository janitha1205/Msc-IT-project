...
function StickyHeadTable({colums,data,apin,api,dis}) {
  ...
  const handleDelete=async (event)=>{
    for(var i=0;i<(rows.length);i++){
        ...
        if (response.ok){
          dispatch2({type: dis,payload: json})
